...
def drop_all_agreement(a):
    ...
    b = b.drop(['number2_auth'], axis=1).to_numpy()
    return b
...
def get_datasets():
    ...
    data_auth = data[data['number2'] == 'auth']
    # 进行数据的拼接
    data = pd.concat([data_ftp_data, data_eco_i])
...
def do_datasets(data_preprocessed):
    ...
    auth_test = x_test[x_test['number2_auth'] == 1]
    ...
    auth_test_x = drop_all_agreement(auth_test)
    auth_test_y = auth_test['type']
    ...
    # auth
    x_train_auth = x_train[x_train['number2_auth'] == 1]
    x_train_auth_p = x_train_auth[x_train_auth['type'] == 1]
    x_train_auth_n = x_train_auth[x_train_auth['type'] == 0]

    train_auth_p = drop_all_agreement(x_train_auth_p)
    auth_p_label = x_train_auth_p['type'].to_numpy()

    train_auth_n = drop_all_agreement(x_train_auth_n)
    auth_n_label = x_train_auth_n['type'].to_numpy()
    ...
    train_wen = np.concatenate(
        [train_ftp_data_p, train_ftp_data_n, train_telnet_p, train_telnet_n,train_other_p, train_other_n, train_finger_p, train_finger_n,train_ftp_p, train_ftp_n, train_private_p, train_private_n,
         train_eco_i_p, train_eco_i_n, train_auth_p, train_auth_n])
    label_wen = np.concatenate(
        (ftp_data_p_label, ftp_data_n_label, telnet_p_label, telnet_n_label,other_p_label, other_n_label, finger_p_label, finger_n_label,ftp_p_label, ftp_n_label, private_p_label, private_n_label,
         eco_i_p_label, eco_i_n_label, auth_p_label, auth_n_label))

    train_wen_p = np.concatenate(
        [train_ftp_data_p,train_telnet_p,train_other_p,train_finger_p,train_ftp_p,train_private_p,train_eco_i_p,train_auth_p])
    label_wen_p = np.concatenate(
        (ftp_data_p_label, telnet_p_label,other_p_label, finger_p_label,ftp_p_label, private_p_label,eco_i_p_label, auth_p_label))
    ...
    return x_train_ftp_data_p,x_train_ftp_data_n, x_train_telnet_p,x_train_telnet_n, \
           ...
           x_train_eco_i_p, x_train_eco_i_n, x_train_auth_p, x_train_auth_n, \
           ...
           eco_i_test_x, auth_test_x, eco_i_test_y, auth_test_y, \
           test_x,test_label,\
           x_train,\
...
def fairness():
    ...
    loss_auth_p = wi0_auth_p = np.ones(x_train_auth_p.shape[0])
    loss_auth_n = wi0_auth_n = np.ones(x_train_auth_n.shape[0])
    ...
    train_wen = np.concatenate(
        [train_ftp_data_p, train_ftp_data_n, train_eco_i_p, train_eco_i_n, train_other_p, train_other_n,
         train_private_p, train_private_n, train_finger_p, train_finger_n, train_telnet_p, train_telnet_n,
         train_ftp_p, train_ftp_n, train_auth_p, train_auth_n])
    label_wen = np.concatenate(
        (ftp_data_p_label, ftp_data_n_label, eco_i_p_label, eco_i_n_label, other_p_label, other_n_label,
         private_p_label, private_n_label, finger_p_label, finger_n_label, telnet_p_label, telnet_n_label,
         ftp_p_label, ftp_n_label, auth_p_label, auth_n_label))
    ...
    while dif(wi0, wi1) > 0.0001 and mm<20:
        ...
        # auth
        for i2 in range(train_auth_n.shape[0]):
            train_auth_n[i2] = -math.log(log_reg.predict_proba(train_auth_n)[i2][0] + 1e-100)

        for i1 in range(train_auth_p.shape[0]):
            loss_auth_p[i1] = -math.log(log_reg.predict_proba(train_auth_p)[i1][1] + 1e-100)
        ...
        wi0_auth_p_1 = optim(loss_auth_p, a, c)
        wi0_auth_n_1 = optim(loss_auth_n, a, c)

        wi0 = np.concatenate((wi0_ftp_data_p_1, wi0_ftp_data_n_1, wi0_eco_i_p_1, wi0_eco_i_n_1, wi0_other_p_1,
                              wi0_other_n_1, wi0_private_p_1, wi0_private_n_1, wi0_finger_p_1, wi0_finger_n_1,
                              wi0_telnet_p_1, wi0_telnet_n_1,
                              wi0_ftp_p_1, wi0_ftp_n_1, wi0_auth_p_1, wi0_auth_n_1))
        iter = iter + 1
        ...
        log_reg_pred10 = log_reg.predict(auth_test_x)
        logistic_reg10 = model_eval(auth_test_y, log_reg_pred10)
        log_reg_pred11 = log_reg.predict(test_x)
        logistic_reg11 = model_eval(test_label, log_reg_pred11)

        list_linshi_di = [logistic_reg1['DP'], logistic_reg2['DP'], logistic_reg3['DP'], logistic_reg4['DP'],
                          logistic_reg5[
                              'DP'], logistic_reg8['DP'], logistic_reg9['DP'], logistic_reg10['DP']]
        mean_di = np.mean(list_linshi_di)
        DI = round(100 * abs(
            abs(logistic_reg1['DP'] - mean_di) + abs(logistic_reg2['DP'] - mean_di) + abs(
                logistic_reg3['DP'] - mean_di) + abs(
                logistic_reg4['DP'] - mean_di) + abs(logistic_reg5['DP'] - mean_di) +
            abs(logistic_reg8['DP'] - mean_di) + abs(logistic_reg9['DP'] - mean_di) + abs(
                logistic_reg10['DP'] - mean_di)), 4)
        list_linshi_TNR = [logistic_reg1['TNR'], logistic_reg2['TNR'], logistic_reg3['TNR'], logistic_reg4['TNR'],
                           logistic_reg5[
                               'TNR'], logistic_reg8['TNR'], logistic_reg9['TNR'], logistic_reg10['TNR']]
        mean_TNR = np.mean(list_linshi_TNR)
        DFPR = round(100 * abs(abs(logistic_reg1['TNR'] - mean_TNR) + abs(logistic_reg2['TNR'] - mean_TNR) + abs(
            ...
            logistic_reg9['TNR'] - mean_TNR) + abs(logistic_reg10['TNR'] - mean_TNR)), 4)
        list_linshi_TPR = [logistic_reg1['TPR'], logistic_reg2['TPR'], logistic_reg3['TPR'], logistic_reg4['TPR'],
                           logistic_reg5[
                               'TPR'], logistic_reg8['TPR'], logistic_reg9['TPR'], logistic_reg10['TPR']]
        mean_TPR = np.mean(list_linshi_TPR)
        DFNR = round(100 * abs(abs(logistic_reg1['TPR'] - mean_TPR) + abs(logistic_reg2['TPR'] - mean_TPR) + abs(
            ...
            logistic_reg9['TPR'] - mean_TPR) + abs(logistic_reg10['TPR'] - mean_TPR)), 4)

        prule = round(
            100 * min(logistic_reg1['DP'] / logistic_reg2['DP'], logistic_reg1['DP'] / logistic_reg3['DP'],
                      ...
                      logistic_reg1['DP'] / logistic_reg10['DP'],
                      logistic_reg2['DP'] / logistic_reg1['DP'], logistic_reg2['DP'] / logistic_reg3['DP'],
                      logistic_reg2['DP'] / logistic_reg4['DP'], logistic_reg2['DP'] / logistic_reg5['DP'],
                      logistic_reg2['DP'] / logistic_reg8['DP'], logistic_reg2['DP'] / logistic_reg9['DP'],
                      logistic_reg2['DP'] / logistic_reg10['DP'],
                      logistic_reg3['DP'] / logistic_reg1['DP'], logistic_reg3['DP'] / logistic_reg2['DP'],
                      ...
                      logistic_reg3['DP'] / logistic_reg10['DP'],
                      logistic_reg4['DP'] / logistic_reg1['DP'], logistic_reg4['DP'] / logistic_reg2['DP'],
                      ...
                      logistic_reg4['DP'] / logistic_reg10['DP'],
                      logistic_reg5['DP'] / logistic_reg1['DP'], logistic_reg5['DP'] / logistic_reg3['DP'],
                      logistic_reg5['DP'] / logistic_reg4['DP'], logistic_reg5['DP'] / logistic_reg2['DP'],
                      logistic_reg5['DP'] / logistic_reg8['DP'], logistic_reg5['DP'] / logistic_reg9['DP'],
                      logistic_reg5['DP'] / logistic_reg10['DP'],
                      ...
                      logistic_reg8['DP'] / logistic_reg2['DP'], logistic_reg8['DP'] / logistic_reg9['DP'],
                      logistic_reg8['DP'] / logistic_reg10['DP'],
                      ...
                      logistic_reg9['DP'] / logistic_reg8['DP'], logistic_reg9['DP'] / logistic_reg2['DP'],
                      logistic_reg9['DP'] / logistic_reg10['DP'],
                      logistic_reg10['DP'] / logistic_reg1['DP'], logistic_reg10['DP'] / logistic_reg3['DP'],
                      logistic_reg10['DP'] / logistic_reg4['DP'], logistic_reg10['DP'] / logistic_reg5['DP'],
                      logistic_reg10['DP'] / logistic_reg8['DP'], logistic_reg10['DP'] / logistic_reg9['DP'],
                      logistic_reg10['DP'] / logistic_reg2['DP']), 4)
...
def diyibufen_method(log_reg,name,label,train):
    ...
    list_linshi_di = [logistic_reg1['DP'] , logistic_reg2['DP'] , logistic_reg3['DP'] , logistic_reg4['DP'] , logistic_reg5[
                'DP'], logistic_reg8['DP'] , logistic_reg9['DP'] , logistic_reg10['DP']]
    mean_di = np.mean(list_linshi_di)
    DI = round(100 * abs(
        abs(logistic_reg1['DP']-mean_di)+abs(logistic_reg2['DP']-mean_di)+abs(logistic_reg3['DP']-mean_di)+abs(logistic_reg4['DP']-mean_di)+abs(logistic_reg5['DP']-mean_di)+
        abs(logistic_reg8['DP']-mean_di)+abs(logistic_reg9['DP']-mean_di)+abs(logistic_reg10['DP']-mean_di)), 4)
    list_linshi_TNR = [logistic_reg1['TNR'], logistic_reg2['TNR'], logistic_reg3['TNR'], logistic_reg4['TNR'],
                      logistic_reg5[
                          'TNR'], logistic_reg8['TNR'], logistic_reg9['TNR'], logistic_reg10['TNR']]
    mean_TNR = np.mean(list_linshi_TNR)
    DFPR = round(100 * abs(abs(logistic_reg1['TNR']-mean_TNR)+abs(logistic_reg2['TNR']-mean_TNR)+abs(logistic_reg3['TNR']-mean_TNR)+abs(logistic_reg4['TNR']-mean_TNR)+
                abs(logistic_reg5['TNR']-mean_TNR)+abs(logistic_reg8['TNR']-mean_TNR)+abs(logistic_reg9['TNR']-mean_TNR)+abs(logistic_reg10['TNR']-mean_TNR)), 4)
    list_linshi_TPR = [logistic_reg1['TPR'], logistic_reg2['TPR'], logistic_reg3['TPR'], logistic_reg4['TPR'],
                       logistic_reg5[
                           'TPR'], logistic_reg8['TPR'], logistic_reg9['TPR'], logistic_reg10['TPR']]
    mean_TPR = np.mean(list_linshi_TPR)
    DFNR = round(100 * abs(abs(logistic_reg1['TPR']-mean_TPR)+abs(logistic_reg2['TPR']-mean_TPR)+abs(logistic_reg3['TPR']-mean_TPR)+abs(logistic_reg4['TPR']-mean_TPR)+
                           abs(logistic_reg5['TPR'] - mean_TPR) +abs(logistic_reg8['TPR']-mean_TPR)+abs(logistic_reg9['TPR']-mean_TPR)+abs(logistic_reg10['TPR']-mean_TPR)), 4)

    prule = round(100 * min(logistic_reg1['DP'] / logistic_reg2['DP'], logistic_reg1['DP'] / logistic_reg3['DP'],
                            logistic_reg1['DP'] / logistic_reg4['DP'], logistic_reg1['DP'] / logistic_reg5['DP'],
                            logistic_reg1['DP'] / logistic_reg8['DP'], logistic_reg1['DP'] / logistic_reg9['DP'],
                            logistic_reg1['DP'] / logistic_reg10['DP'],
                            logistic_reg2['DP'] / logistic_reg1['DP'], logistic_reg2['DP'] / logistic_reg3['DP'],
